...
# MongoDB connection (Atlas URI from environment variable for safety)
MONGO_URI = os.getenv("MONGO_URI", "mongodb+srv://akshatmaggon1_db:Akshat1234@attendance.xndiua8.mongodb.net/?retryWrites=true&w=majority&appName=attendance")
...
# Helpers
def hash_pw(password): return bcrypt.hashpw(password.encode(), bcrypt.gensalt())
def verify_pw(password, hashed): return bcrypt.checkpw(password.encode(), hashed)
def create_token(user_id): 
    payload = {"user_id": str(user_id), "exp": datetime.datetime.utcnow() + datetime.timedelta(days=1)}
...
def create_class(name: str, token: str):
    ...
    res = db.classes.insert_one({"name": name, "createdBy": decoded["user_id"], "joinCode": join_code})
    db.members.insert_one({"classId": res.inserted_id, "userId": decoded["user_id"], "role": "teacher"})
...
def join_class(joinCode: str, token: str):
    ...
    db.members.insert_one({"classId": cls["_id"], "userId": decoded["user_id"], "role": "student"})
...
def start_session(classId: str, token: str, request: Request):
    ...
    # Use the host domain dynamically (works locally & on Render)
    base_url = str(request.base_url).rstrip("/")
    qr_data = f"{base_url}/mark_attendance_qr?sessionId={session_id}&token={token}"
